...
| `RUNNER9` | *(proposed)* a fleet-reachable **Linux** job in a `pull_request` workflow resolves `runs-on` from the host lease, or carries a declared exemption |
| `RUNNER10` | *(proposed)* at most **one** job across a repository's `pull_request` workflows declares `services:` — infrastructure is brought up once per pull request |
