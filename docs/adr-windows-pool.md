...
* the per-job slot reset not installed — any of its service, its hooks, or the
  profile template the reset restores from (§3 phase 4, revised by #232);
...
> retired the equivalent hook in #110 and replaced it in #231 — #237 records the
> follow-up findings against that replacement, and is not the replacement — for a
...
cannot ask for a reset of a slot that is not itself. (The callers that *do*
supply an index are the ones already running as root, with no `sudo` in the
picture and so no `SUDO_UID` to read: systemd's `ExecStartPre`, the slot
provisioner, and the teardown and sweeper paths. Root may name any slot; that
is the point of the split.) Windows has neither half — no `sudo` to prove who is asking,
