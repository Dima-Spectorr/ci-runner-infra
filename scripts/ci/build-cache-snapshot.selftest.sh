...
has_inherited_std_handles() { # <file>
  ...
  code=$(code "$1")
  printf '%s' "$code" | grep -q 'UseStdHandles = 0x00000100' || return 1
  printf '%s' "$code" | grep -q 'si.Flags = UseStdHandles;' || return 1
  printf '%s' "$code" | grep -q 'si.StdOutput = Usable(GetStdHandle(StdOutputHandle));' || return 1
  printf '%s' "$code" | grep -q 'si.StdError = Usable(GetStdHandle(StdErrorHandle));' || return 1
  printf '%s' "$code" | grep -q 'CreateProcess(null, line, IntPtr.Zero, IntPtr.Zero, true,' || return 1
  ...
  printf '%s' "$code" | grep -q 'handle == InvalidHandle ? IntPtr.Zero : handle' || return 1
  flags_line=$(grep -n 'si.Flags = UseStdHandles;' "$1" | head -1 | cut -d: -f1)
  create_line=$(grep -n 'CreateProcess(null, line, IntPtr.Zero, IntPtr.Zero, true,' "$1" | head -1 | cut -d: -f1)
...
has_hardlink_flattening() { # <file>
  ...
  code "$1" | grep -q 'Resolve-StagedHardLink -Files $files' || return 1
  ...
  code "$1" | grep -q 'if ($links -lt 0) {' || return 1
  code "$1" | grep -q 'return -1;' || return 1
  # BEFORE TAR, because tar is what records the link.
  local flat pack
  flat=$(code "$1" | grep -n 'Resolve-StagedHardLink -Files $files' | head -1 | cut -d: -f1)
  pack=$(code "$1" | grep -n 'tar -czf $out -C $stage' | head -1 | cut -d: -f1)
...
has_unquiesced_stage_kept() { # <file>
  ...
  code "$1" | grep -q '$script:StageNotQuiesced = $true' || return 1
  code "$1" | grep -q 'if ($script:StageNotQuiesced) {' || return 1
  # And the flag is read BEFORE the recursive delete, or it is a record of
  # something that already happened.
  local flag del
  flag=$(code "$1" | grep -n 'if ($script:StageNotQuiesced) {' | head -1 | cut -d: -f1)
  del=$(code "$1" | grep -n 'Remove-Item -LiteralPath $Path -Recurse' | head -1 | cut -d: -f1)
...
has_credential_name_scan() { # <file>
  ...
  code "$1" | grep -q 'Get-CredentialFileRefusal -Entries $entries' || return 1
  ...
  code "$1" | grep -q 'if ($credential) {' || return 1
  code "$1" | grep -q 'throw "refusing to pack: $credential"' || return 1
  # ORDER: before the archive is written, or it is a refusal of a file that has
  # already been packed.
  local cred pack
  cred=$(code "$1" | grep -n 'Get-CredentialFileRefusal -Entries $entries' | head -1 | cut -d: -f1)
  pack=$(code "$1" | grep -n 'tar -czf $out -C $stage' | head -1 | cut -d: -f1)
