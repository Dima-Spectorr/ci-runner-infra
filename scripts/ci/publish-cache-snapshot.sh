...
# The embedded-credential CONTENT pass, and the digest allowlist that excuses a
# dependency's own PEM test fixture. Sourced, not run: this script keeps its own
# `die`, `log` and `safe_path`, and the library defers to them.
#
# Sourced HERE, before the prepare command runs, and deliberately so. The build
# job's checkout is writable by whatever the prepare command installs; a source
# that happened after it would be reading a file that untrusted install code had
...
if ! declare -F scan_credentials_or_die >/dev/null 2>&1; then
  . "$HERE/scan-cache-credentials.sh"
fi
declare -F scan_credentials_or_die >/dev/null 2>&1 \
  || die "the embedded-credential content scan is not loaded — refusing to run a publish that cannot scan"
...
# Where the content pass creates its hit list. Inside $ARCHIVE_DIR, which is a
# fresh 0700 mktemp -d, so the list is unreachable to anything but this uid —
...
# --- the scan --------------------------------------------------------------------
#
# The passes that are meaningful only here — a link, a device node, a setuid
# bit, a file capability, a credential FILENAME — stay in this file. The
# EMBEDDED-CREDENTIAL CONTENT pass does not: `CREDENTIAL_PATTERNS`,
# `URL_SCHEME_ALT`, the PEM confirmation stage, the byte floor and the digest
# allowlist all live in scan-cache-credentials.sh, sourced above, so the
# Windows builder runs one copy of them rather than a translation.
...
scan_or_die() { # <tree>
  ...
  # The content pass itself lives in scan-cache-credentials.sh, sourced above.
  # It is a separate file because the Windows builder has to run the SAME
  # patterns over its own staged tree, and a PowerShell port would be a third
  # copy of security-critical rules that all have to agree.
  scan_credentials_or_die "$root"
}
